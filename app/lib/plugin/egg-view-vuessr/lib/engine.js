...
class Engine {
  constructor(app) {
    ...
    if (layout && fs.existsSync(layout)) {
      this.template = fs.readFileSync(layout, 'utf-8');
  ...
  createBundleRenderer(name, template, renderOptions) {
    if (this.bundleCache) {
    ...
    const clientManifest = require(this.config.manifest)
    const bundleRenderer = this.vueServerRenderer.createBundleRenderer(name, Object.assign({
      template,
      clientManifest
